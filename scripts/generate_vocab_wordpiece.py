...
flags.DEFINE_string(
    "lang_set",
    "en,es,ru,ar,de,fr,it,pt,ja,pl,fa,zh",
    "Set of languages used to build wordpiece model, " "given as a comma-separated list.",
)
flags.DEFINE_string("text_key", "text", "Text feature key in input examples.")
flags.DEFINE_string("language_code_key", "language_code", "Language code feature key.")
flags.DEFINE_float("smoothing_exponent", 0.5, "Exponent used in calculating exponential smoothing coefficients.")
flags.DEFINE_integer("max_word_length", 50, "Discard words of length greater than max_word_length.")
flags.DEFINE_integer("upper_thresh", 10000000, "Upper threshold for binary search.")
flags.DEFINE_integer("lower_thresh", 10, "Lower threshold for binary search.")
flags.DEFINE_integer("num_iterations", 4, "Number of iterations in wordpiece learning algorithm.")
flags.DEFINE_integer("num_pad_tokens", 100, "Number of padding tokens to " "include in vocab.")
flags.DEFINE_integer("max_input_tokens", 5000000, "Maximum number of input tokens, where -1 means no max.")
flags.DEFINE_integer("max_token_length", 50, "Maximum length of a token.")
flags.DEFINE_integer("max_unique_chars", 1000, "Maximum number of unique characters as tokens.")
flags.DEFINE_integer(
    "vocab_size",
    110000,
    "Target size of generated vocab, "
    "where vocab_size is an upper bound and the size of vocab "
    "can be within slack_ratio less than the vocab_size.",
)
flags.DEFINE_float("slack_ratio", 0.05, "Difference permitted between target and actual vocab size.")
flags.DEFINE_bool("include_joiner_token", True, "Whether to include joiner token in word suffixes.")
flags.DEFINE_string("joiner", "##", "Joiner token in word suffixes.")
flags.DEFINE_list(
    "reserved_tokens",
    ["<unk>", "<s>", "</s>", "<mask>", "<cls>", "<sep>", "<S>", "<T>"],
    "Reserved tokens to be included in vocab.",
)


def generate_vocab(data_file, vocab_file, metrics_file, raw_metadata, params, min_token_frequency=2):
    ...
    example_converter = tft.coders.ExampleProtoCoder(raw_metadata.schema, serialized=False)
    ...
    def run_vocab():
        ...
        with tft_beam.Context(temp_dir=tempfile.mkdtemp()):
            ...
            raw_data = (
                vocab_pipeline
                | "ReadInputData"
                >> beam.io.tfrecordio.ReadFromTFRecord(data_file, coder=beam.coders.ProtoCoder(tf.train.Example))
            ...
            (transformed_data, _), _ = (
                ...
                utils.count_preprocessing_fn(FLAGS.text_key, FLAGS.language_code_key)
            )

            # Filter by languages.
            tokens = transformed_data | "FilterByLang" >> beam.ParDo(utils.FilterTokensByLang(lang_set))

            # Calculate smoothing coefficients.
            coeffs = tokens | "CalculateSmoothingCoefficients" >> beam.CombineGlobally(
            ...
            _ = (
                tokens
                | "ApplyExponentialSmoothing" >> beam.ParDo(utils.ExponentialSmoothing(), beam.pvalue.AsSingleton(coeffs))
                | "SumCounts" >> beam.CombinePerKey(sum)
                | "FilterLowCounts" >> beam.ParDo(utils.FilterByCount(FLAGS.max_word_length, min_token_frequency))
                | "MergeAndSortCounts" >> beam.CombineGlobally(utils.SortByCount())
                | "LearnVocab" >> beam.ParDo(utils.LearnVocab(params))
                | "Flatten" >> beam.FlatMap(lambda x: x + "\n")
                | "WriteVocab" >> beam.io.WriteToText(vocab_file, shard_name_template="", append_trailing_newlines=False)
            )
    ...
    def run_metrics():
        ...
        with tft_beam.Context(temp_dir=tempfile.mkdtemp()):
            ...
            (metrics_transformed_data, _), _ = (
                ...
                utils.metrics_preprocessing_fn(FLAGS.vocab_file, FLAGS.text_key, FLAGS.language_code_key)
            )
            ...
            _ = (
                ...
                | "WriteMetrics" >> beam.io.WriteToText(metrics_file, shard_name_template="", header=",".join(columns))
            )
...
def main(_):
    ...
    params = learner.Params(
        ...
        FLAGS.max_input_tokens,
        FLAGS.max_token_length,
        FLAGS.max_unique_chars,
        FLAGS.vocab_size,
        FLAGS.slack_ratio,
        FLAGS.include_joiner_token,
        FLAGS.joiner,
        reserved_tokens,
    )

    generate_vocab(FLAGS.data_file, FLAGS.vocab_file, FLAGS.metrics_file, raw_metadata, params)
